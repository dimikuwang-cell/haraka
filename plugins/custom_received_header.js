...
exports.register = function () {
    ...
    // 加载配置文件
    this.cfg = this.config.get('custom_received_header.ini', {
...
exports.generateCustomReceivedHeader = function (connection, transaction) {
    ...
    // 获取收件人（如果有多个取第一个）
    const rcpt_to = transaction.rcpt_to && transaction.rcpt_to.length > 0 
    ...
    const received = `from ${config.from_domain} (${config.from_hostname} [${config.from_ip}])` + String.fromCharCode(13, 10) +
                     `\tby ${config.by_hostname} (${config.by_info}) with SMTP id ${smtp_id}` + String.fromCharCode(13, 10) +
